...
1. **Select the change** — run `node .agent/skills/spec-driven-archive/scripts/spec-driven.js modify` to list active changes. Ask which change to archive. If already specified, use it.

2. **Check for incomplete tasks** — run:
   ```
   node .agent/skills/spec-driven-archive/scripts/spec-driven.js apply <name>
   ```
