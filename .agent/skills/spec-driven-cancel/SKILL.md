...
1. **Select the change** — run `node .agent/skills/spec-driven-cancel/scripts/spec-driven.js modify` to list active changes. Ask which change to cancel. If already specified, use it.
...
3. **Cancel the change** — run:
   ```
   node .agent/skills/spec-driven-cancel/scripts/spec-driven.js cancel <name>
   ```
