...
1. **Select the change** — run `node dist/scripts/modify.js` to list active changes. Ask which change to cancel. If already specified, use it.
...
3. **Cancel the change** — run:
   ```
   node dist/scripts/cancel.js <name>
   ```
